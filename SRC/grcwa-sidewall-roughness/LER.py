...
class edge_roughness:
    # analyse an image
    def LER_analysis(
        self, image, im_wdth, thresh, show=False
    ):  # origional image, width in meters of image (ie the scale), threshold for outliers
        bin_im = self.binarization(image)  # binarize the image

        cv2.imwrite("binary_test.jpg", bin_im)  # write the binary image to a file

        res = im_wdth[1] / bin_im.shape[1]
        print("resolution: ", res)

        X = np.linspace(
            0, im_wdth, num=bin_im.shape[1], endpoint=True
        )  # create the X data array, width of image and number pixels
        Y = self.binary_imsearch(255, bin_im)  # find the line edge values

        if show:
            plt.plot([x * 1e6 for x in X], [y * res for y in Y], "r", label="raw")
        ...
        Xcln, Ycln = self.extract_clean(
            X, Y, thresh
        )  # extract the cleaned data from the raw edge data

        Ycln = [y * res for y in Ycln]

        if show:
            try:
                plt.plot([x * 1e6 for x in Xcln], Ycln, "b", label="cleaned")
                plt.legend()
                plt.show()
            except:
                plt.show()
    ...
    def binarization(self, image):
        try:
            ...
            )  # Convert the loaded image to greyscale, only required for openCV version 2 I am using 3 (confusingly also imoprts as cv2, as it's in beta)
        except:
            pass
        ...
        )  # Otsu method following filtering
        retBin, binary = cv2.threshold(
            blur, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
        )  # repeat above, oddly can't just copy above to new variable for storage
    ...
    def remove_outliers(self, Xdata, Ydata, n):
        ...
        finterp = interp1d(
            XdataClean, YdataClean, fill_value="extrapolate"
        )  # added fill_value='extrapolate' - JK 12/12/23
    ...
    def remove_background(self, Xdata, Ydata):  # x data, ydata
        ...
        print(
            "Mean squared error: %.2f" % np.mean((regr.predict(Xdata) - Ydata) ** 2)
        )  # mean squared error
        print(
            "Variance score: %.2f" % regr.score(Xdata, Ydata)
